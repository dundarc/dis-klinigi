...
class TestOCRCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Test the OCR service with a sample invoice';
...
Ödenecek Tutar 20.083,38 TL";
        ...
        $tempFile = tempnam(sys_get_temp_dir(), 'ocr_test');
        file_put_contents($tempFile, $invoiceContent);

        try {
            // Test the OCR service
            $ocrService = new OCRService();
            $this->info('OCR Service loaded successfully');
            ...
            $this->info('OCR Parsing completed');
            $this->line('File size: ' . filesize($tempFile));
            $this->line('File content length: ' . strlen($invoiceContent));
            $this->line('First 200 chars: ' . substr($invoiceContent, 0, 200));
            $this->line('Invoice Number: ' . ($result['invoice_number'] ?? 'Not found'));
            $this->line('Invoice Date: ' . ($result['invoice_date'] ?? 'Not found'));
            $this->line('Grand Total: ' . ($result['grand_total'] ?? 'Not found'));
            $this->line('Line Items Count: ' . count($result['line_items'] ?? []));
            $this->line('Supplier Info: ' . print_r($result['supplier_info'] ?? 'Not found', true));
            $this->line('Confidence Score: ' . ($result['confidence_score'] ?? 'Not found'));
            
            if (!empty($result['line_items'])) {
                $this->line('First Line Item: ' . print_r($result['line_items'][0], true));
            }
            
            // Show full result for debugging
            $this->info('Full OCR Result:');
