<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Yeni E-posta Şablonu</h2>
                <p class="text-slate-600 dark:text-slate-400 mt-1">Yeni bir e-posta şablonu oluşturun</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('system.email.templates.store') }}" method="POST" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Şablonu kod içinde çağırmak için kullanılan benzersiz anahtar</p>
                        @error('key')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="body_html" name="body_html" rows="10"
                                  class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:text-slate-200"
                                  placeholder="<h1>Merhaba Ahmet Yılmaz</h1>">{{ old('body_html') }}</textarea>
                        @error('body_html')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="body_text" name="body_text" rows="5"
                                  class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-slate-700 dark:text-slate-200"
                                  placeholder="Merhaba Ahmet Yılmaz, randevunuz 15 Ocak 2025 tarihinde saat 14:00'te">{{ old('body_text') }}</textarea>
