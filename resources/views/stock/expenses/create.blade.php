<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Yeni Gider</h2>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <x-card class="space-y-6">
                <form method="POST" action="{{ route('stock.expenses.store') }}" class="space-y-6">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <x-input-label for="title" value="Başlık" />
                            <x-text-input id="title" name="title" type="text" value="{{ old('title') }}" class="mt-1 block w-full" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="expense_date" name="expense_date" type="date" value="{{ old('expense_date', now()->toDateString()) }}" class="mt-1 block w-full" />
                        </div>
                        <div>
                            <x-input-label for="amount" value="Tutar" />
                            <x-text-input id="amount" name="amount" type="number" step="0.01" value="{{ old('amount') }}" class="mt-1 block w-full" required />
                        </div>
                        <div>
                            <x-input-label for="vat_rate" value="KDV %" />
                            <x-text-input id="vat_rate" name="vat_rate" type="number" step="0.01" value="{{ old('vat_rate', 0) }}" class="mt-1 block w-full" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="paid_at" name="paid_at" type="date" value="{{ old('paid_at') }}" class="mt-1 block w-full" />
                        </div>
