<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-900 dark:text-slate-50">{{ __('Yeni Gider Kategorisi') }}</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Yeni bir gider kategorisi oluşturun') }}</p>
            </div>
    ...
    <div class="py-10">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <x-card>
                <form method="POST" action="{{ route('stock.expense-categories.store') }}">
                    @csrf
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <x-primary-button type="submit">{{ __('Oluştur') }}</x-primary-button>
                    </div>
                </form>
            </x-card>
        </div>
