<x-app-layout>
    ...
    <div class="py-12" x-data="visitActionManager({
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-600 dark:bg-amber-900/30 dark:text-amber-200">
                Muayene tamamlandığında ziyaret durumunu <strong>“Tamamlandı”</strong> olarak güncellemeyi unutmayın. Ziyaret tamamlanmadıkça hasta bekleme listesinde görünmeye devam eder.
            </div>
                ...
                <form method="POST" action="{{ route('waiting-room.action.update', $encounter) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <x-select-input id="status" name="status" class="mt-1 block w-full" required>
                                @foreach($statuses as $status)
                                    @if(!in_array($status, [\App\Enums\EncounterStatus::WAITING]))
                                        <option value="{{ $status->value }}" @selected($encounter->status === $status)>{{ __("patient.encounter_status." . $status->value) }}</option>
                                    @endif
                                @endforeach
                            </x-select-input>
                        </div>
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm">{{ old('notes', $encounter->notes) }}</textarea>
                        </div>
                    </div>
                                ...
                                @foreach($encounter->treatments as $treatment)
                                    <li class="rounded border border-gray-200 px-3 py-2 dark:border-gray-700">
                                        ...
                                        <p class="mt-1 font-medium text-gray-900 dark:text-gray-100">
                                            {{ $treatment->treatment?->name ?? 'Tedavi' }}
                            ...
                            <template x-for="(treatment, index) in appliedTreatments" :key="index">
                                <div class="flex flex-col gap-3 rounded-lg border border-gray-200 p-3 dark:border-gray-700 md:flex-row md:items-end">
                                    <div class="md:flex-1">
                                        ...
                                        <x-select-input x-bind:name="`treatments[${index}][treatment_id]`" @change="updatePrice(index, $event.target)" class="mt-1 w-full">
                                    ...
                                    <div class="md:w-28">
                                        ...
                                        <input type="number" x-bind:name="`treatments[${index}][tooth_number]`" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm">
                                    </div>
                                    <div class="md:w-36">
                                        <x-input-label value="Ücret" />
                                        <input type="number" step="0.01" x-bind:name="`treatments[${index}][unit_price]`" x-model="treatment.unit_price" class="mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm">
                                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="prescription_text" name="prescription_text" rows="5" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm" placeholder="İlaç ve kullanım talimatlarını yazın."></textarea>
                    </div>
                                ...
                                <li class="flex flex-col rounded border border-gray-200 px-3 py-2 dark:border-gray-700">
                                    <div class="flex flex-wrap items-center justify-between gap-2">
                                        <div class="font-medium text-gray-900 dark:text-gray-100">
                                            <a href="{{ Storage::url($file->file_path) }}" target="_blank" class="text-indigo-600 hover:underline dark:text-indigo-300">
                    ...
                    <form class="space-y-3" @submit.prevent="uploadFile">
                        <div class="grid gap-3 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input id="file_notes" name="notes" type="text" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm" x-model="fileForm.notes" placeholder="Opsiyonel açıklama">
                            </div>
                        </div>
                        <div>
                            <x-input-label for="visit_file" value="Dosya Seç" />
                            <input id="visit_file" name="file" type="file" class="mt-1 block w-full text-sm text-gray-700 dark:text-gray-300" @change="setFile($event)">
                        </div>
