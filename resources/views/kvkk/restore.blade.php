<x-app-layout>
    ...
    <div x-data="kvkkRestore">
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                    <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Toplu İşlemler</h3>
                    <div class="flex gap-4">
                        ...
                        <button @click="showBulkRestoreModal = true"
                        ...
                        <button @click="showBulkHardDeleteModal = true"
                                x-show="selectedPatients.length > 0"
        ...
        <!-- Bulk Restore Modal -->
        <div x-show="showBulkRestoreModal" x-cloak
             class="fixed inset-0 z-50 overflow-y-auto"
             @keydown.escape.window="showBulkRestoreModal = false">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form action="{{ route('kvkk.bulk-restore') }}" method="POST" x-data="{ confirmed: false }">
                        @csrf
                        <input type="hidden" name="patient_ids" x-bind:value="selectedPatients.join(',')">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-slate-500 dark:text-slate-400">
                                            Seçilen <strong><span x-text="selectedPatients.length"></span> hasta</strong> kaydını geri yüklemek istediğinizden emin misiniz?
                                        </p>
                                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-2">
                                            Bu işlem seçilen tüm hastaların verilerini normal erişime geri döndürecektir.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-slate-50 dark:bg-slate-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            ...
                            <button type="button" @click="showBulkRestoreModal = false"
        ...
        <div x-show="showBulkHardDeleteModal" x-cloak
              class="fixed inset-0 z-50 overflow-y-auto"
              @keydown.escape.window="showBulkHardDeleteModal = false">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity" @click="showBulkHardDeleteModal = false">
                    <div class="absolute inset-0 bg-slate-500 dark:bg-slate-900 opacity-75"></div>
                </div>

                <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form action="{{ route('kvkk.bulk-hard-delete') }}" method="POST" x-data="{ confirmed: false }">
                        @csrf
                        <input type="hidden" name="patient_ids" x-bind:value="selectedPatients.join(',')">
                        <div class="bg-white dark:bg-slate-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <p class="text-sm text-slate-500 dark:text-slate-400">
                                            Seçilen <strong><span x-text="selectedPatients.length"></span> hasta</strong> kaydını kalıcı olarak silmek istediğinizden emin misiniz?
                                        </p>
                                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-2">
                                            Bu işlem tüm randevuları, faturaları, tedavi planlarını ve dosyaları kalıcı olarak silecektir.
                                        </p>
                                        <div class="mt-4">
                                            <div class="flex items-center">
                                                <input id="bulk-hard-delete-confirm" type="checkbox" x-model="confirmed"
                                                       class="h-4 w-4 text-red-600 focus:ring-red-500 border-slate-300 rounded">
                                                <label for="bulk-hard-delete-confirm" class="ml-2 block text-sm text-slate-900 dark:text-slate-100">
                        ...
                        <div class="bg-slate-50 dark:bg-slate-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            ...
                            <button type="button" @click="showBulkHardDeleteModal = false"
                                    class="mt-3 w-full inline-flex justify-center rounded-md border border-slate-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-800 text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                                İptal
                            </button>
                        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('kvkkRestore', () => ({
                ...
                selectAll() {
                    this.selectedPatients = @json($deletedPatients->pluck('id'));
