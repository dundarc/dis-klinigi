<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 md:flex-row md:items-baseline md:justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Takvim: Bug�n</h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="grid gap-6 lg:grid-cols-[2fr,1fr]">
                <div class="space-y-6">
                        ...
                        <form method="GET" action="{{ route('calendar.today') }}" class="space-y-4">
                            <div>
                                ...
                                <div class="mt-3 space-y-2">
                                    ...
                                    @empty
                                        <p class="text-sm text-gray-500 dark:text-gray-400">Tan�ml� hekim bulunmuyor.</p>
                                    @endforelse
                                </div>
                            </div>
                    ...
                    <x-card>
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Bug�n�n Randevular�</h3>
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $appointments->count() }} randevu</span>
                        </div>
                        @if($hourlySlots->isEmpty())
                            <p class="text-sm text-gray-500 dark:text-gray-400">Bug�n i�in randevu bulunmuyor.</p>
                        @else
                            <div class="space-y-5">
                ...
                <div class="space-y-6">
                    <x-card>
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Acil S�ra Bekleyenler</h3>
                            <x-secondary-button-link href="{{ route('waiting-room.emergency') }}">
                                T�m�n� G�r
                            </x-secondary-button-link>
                        </div>
                        @if($emergencyEncounters->isEmpty())
                            <p class="text-sm text-gray-500 dark:text-gray-400">�u anda acil bekleyen hasta bulunmuyor.</p>
                        @else
                            <div class="space-y-3">
                                        ...
                                        <div class="flex flex-wrap items-center justify-between gap-2">
                                            <div>
                                                ...
                                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                                    Triyaj: {{ $triageLabel }} � Geli�: {{ $arrivalTime }}
                                                </p>
                                            </div>
