<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Günlük Takvim</h2>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Günlük Görünüm</h3>
                                <p class="text-sm text-slate-600 dark:text-slate-400">Günlük randevu takibi</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 no-print">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400">Günlük randevuları filtreleyin</p>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 no-print">
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        {{-- Date Picker --}}
                        <form method="GET" action="{{ route('calendar.day') }}" class="flex items-center gap-2">
                            <input type="hidden" name="view" value="{{ $currentView }}">
                            ...
                            @endforeach
                            <div class="flex items-center gap-2">
                                <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Tarih:</label>
                                <input type="date" name="goto_date" value="{{ $gotoDate }}"
                                       class="rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm shadow-sm transition-colors hover:border-slate-400 dark:hover:border-slate-500 focus:border-blue-500 focus:ring-blue-500">
                                <button type="submit"
            ...
            {{-- Modern Daily Schedule --}}
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                {{-- Day Header --}}
                @foreach($days as $day)
                    <div class="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 bg-blue-100 dark:bg-blue-900/50 rounded-xl flex items-center justify-center">
                                    <span class="text-lg font-bold text-blue-600 dark:text-blue-400">{{ $day['date']->format('j') }}</span>
                                </div>
                                <div>
                                    <h3 class="text-xl font-bold text-slate-900 dark:text-slate-100">{{ $day['date']->format('l, d F Y') }}</h3>
                                    <p class="text-sm text-slate-600 dark:text-slate-400">{{ $day['date']->format('F Y') }}</p>
                                </div>
                            </div>
                            @if($day['isToday'])
                ...
                <div class="divide-y divide-slate-200 dark:divide-slate-600">
                    ...
                    @foreach($hours as $hour)
                        @php
                            $day = $days[0]; // Only one day in day view
                            $hourData = $day['hours']->firstWhere('hour', $hour);
                            $hasAppointments = $hourData && $hourData['appointments']->count() > 0;
                        @endphp

                        <div class="px-6 py-4 hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors {{ $hasAppointments ? 'bg-blue-50/30 dark:bg-blue-900/10' : '' }}">
